function RunOptimizationALCLOSURE(params, filename, targetsDir)
% run optimization on motor pattern with 2 targets for CLOSURE
...
while(done ==0)    
    ...
    while(initDone==0)
        ...
        % contraint on multiple touch points stricter at front of mouth
        if(minCSAIdx >= 15)
            pruneContactTheshold=2;
        else
            pruneContactTheshold=5;
        end
        
        % if no contacts then fail
        % if too many contacts then fail
        if( contactCount >= 1 && contactCount < pruneContactTheshold)
            initDone=1;
        end
    end    
    ...
    if(  (contactCount >= 1)  && contactCount < pruneContactTheshold  )
        ...
        if(contactCount < 1 )
            disp(sprintf('Abandoned because: No Contact = %g',contactCount));
        end
        if(contactCount >= pruneContactTheshold)
            disp(sprintf('Abandoned because: Too many contacts = %g',contactCount));
        end
