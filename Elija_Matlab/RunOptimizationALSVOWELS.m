function RunOptimizationALSVOWELS(params, filename, targetsDir)
...
while(done ==0)    
    ...
    while(initDone==0)
        ...
        [cost, sensoryConsequences] = objfunFull2TALSCore(aparam_init, params.libname, params);
    ...
    if(params.optimizationCycles == 0)
        ...
        % run optimization
         ...
         Aeq=[];
         beq=[];          
         [aparam_found, fval, exitflag, output] = fmincon(@objfunFull2TALSCore, aparam_init, A, b, Aeq, beq, lb,ub, @nonlcon, options, params.libname, params)
    end
    
    % run objective function again to get sensory consequences
	[cost, sensoryConsequences] = objfunFull2TALSCore(aparam_found, params.libname, params);
    ...
    if(  (contactCount < 1)  && ...
         (sensoryConsequences.motorPatternSensitivity < params.motorPatternSensitivityThresh) )
        ...
        %if( abs(sumLFversusHFratio) <= abs(sumHFversusLFratio) )
        %    disp(sprintf('Abandoned because: LF:%g  <=  HF:%g',sumLFversusHFratio, sumHFversusLFratio));
        %end
        ...
        if( sensoryConsequences.motorPatternSensitivity >= params.motorPatternSensitivityThresh )
            disp(sprintf('Abandoned because: motorPatternSensitivity = %g',sensoryConsequences.motorPatternSensitivity));
        end        
    end
